...
/-
Here's the key idea. If a term contains,
or is parameterized by, a value in Type
u, where u is any natural number, then
it must live (at least) in Type (u+1).


In the earlier example, (a := nat : Type),
so (α := Type : Type 1)
-/
